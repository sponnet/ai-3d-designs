...
the mount's own back-plate hole pattern on one side, and a 180° half-shell
that cradles half the tube on the other side. **Two** of these pieces
(the 2nd installed rotated 180° around the tube) sandwich the tube; each
piece has a flange with 2 holes at **both** ends of its half-shell, so the
2 pieces bolt together at both sides of the tube (4 bolts total for the
full clamp) — a standard 2-piece pipe-clamp arrangement.
...
- Plate sits centered (`Y=0`) on top of the shell, standing perpendicular
  to it, so the BnD mount ends up offset above (or, for the 2nd/rotated
  piece, below) the tube.
