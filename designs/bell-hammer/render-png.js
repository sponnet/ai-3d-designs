/**
 * Export bell-hammer.jscad -> bell-hammer.stl -> bell-hammer.png.
 * Run from repo root: node designs/bell-hammer/render-png.js
 */
const fs = require('fs')
const path = require('path')
const { execSync } = require('child_process')
const { stl2png } = require('@scalenc/stl-to-png')
const { PRETTY_RENDER_OPTIONS } = require('../render-style')

const designDir = __dirname
const repoRoot = path.join(designDir, '..', '..')
const jscadFile = path.join(designDir, 'bell-hammer.jscad')
const stlPath = path.join(designDir, 'bell-hammer.stl')
const pngPath = path.join(designDir, 'bell-hammer.png')

if (!fs.existsSync(jscadFile)) {
  console.error('Missing bell-hammer.jscad in', designDir)
  process.exit(1)
}

execSync(`npx jscad "${jscadFile}" -o "${stlPath}"`, { cwd: repoRoot, stdio: 'inherit' })

const stlBuf = fs.readFileSync(stlPath)
const pngBuf = stl2png(stlBuf, PRETTY_RENDER_OPTIONS)
fs.writeFileSync(pngPath, pngBuf)
console.log('Wrote', pngPath)
