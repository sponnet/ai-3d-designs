...
const getParameterDefinitions = () => [
  ...
  { name: 'landingThickness', type: 'float', initial: DEFAULTS.landingThickness, caption: 'Landing thickness:' },
  ...
  { name: 'renderGlueTabs', type: 'checkbox', checked: DEFAULTS.renderGlueTabs, caption: 'Render glue tabs' },
...
const landing = (p, d) => {
  ...
  return translate(
    [p.stairWidth / 2, d.firstFlightRun + d.landingDepth / 2, z - p.landingThickness / 2],
    cuboid({ size: [p.stairWidth, d.landingDepth, p.landingThickness] })
  )
}
...
const staircaseWithLanding = (p, d) => {
  ...
  if (p.renderPart1) {
    ...
    if (p.renderGlueTabs) solids.push(colorize([0.20, 0.60, 0.95, 1], firstFlightGlueTabs(p, d)))
    solids.push(colorize([0.95, 0.75, 0.20, 1], landing(p, d)))
  }

  if (p.renderPart2) {
    if (p.renderGlueTabs) solids.push(colorize([0.15, 0.45, 0.85, 1], secondFlightGlueTabs(p, d)))
